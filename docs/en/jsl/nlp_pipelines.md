...
Spark ML provides a set of Machine Learning applications that can be build using two main components: **Estimators** and **Transformers**. The **Estimators** have a method called fit() which secures and trains a piece of data to such application. 
...
The basic result of a NLP operation is an **annotation**. It's structure includes:
...
spark = nlp.start()
finisher = nlp.Finisher().setInputCols(["token", "lemmas", "pos"])
explain_pipeline_model = nlp.PretrainedPipeline("explain_document_ml").model
...
from johnsnowlabs import nlp
explain_document_pipeline = nlp.PretrainedPipeline("explain_document_ml")
lightPipeline = nlp.LightPipeline(explain_document_pipeline.model)
...
```python
from johnsnowlabs import nlp
nlp.LightPipeline(someTrainedPipeline).annotate(someStringOrArray)
```
...
```python
from johnsnowlabs import nlp 
recursivePipeline = nlp.RecursivePipeline(stages=[
    documentAssembler,
    sentenceDetector,
    tokenizer,
    lemmatizer,
