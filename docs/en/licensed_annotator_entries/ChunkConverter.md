...
val empty_df= Seq("") .toDF("text") model= pipeline.fit(empty_df) 
val lp_model = new LightPipeline(model)
 
val results = lp_model.fullAnnotate(sample_text) Array(0)
...
val sample_text="""AWA Group LP intends to pay dividends on the Common Units on a quarterly basis at an annual rate of 8.00% of the Offering Price. """ 
