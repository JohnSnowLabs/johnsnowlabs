...
setCaseSensitive(): This parameter is used to decide whether to lowercase tokens or not during the token classification. If it is set as True, it considers the casing status of the tokens. If it is set as False, it lowercases the tokens so the casing status of the tokens are not considered.
{%- endcapture -%}
...
|index|sentence\_id|chunk|end|ner\_label|
|---|---|---|---|---|
|0|0|gestational diabetes mellitus|67|PROBLEM|
|1|0|type two diabetes mellitus|153|PROBLEM|
|2|0|T2DM|160|PROBLEM|
|3|0|HTG-induced pancreatitis|209|PROBLEM|
|4|0|an acute hepatitis|280|PROBLEM|
|5|0|obesity|294|PROBLEM|
|6|0|a body mass index|317|TEST|
|7|0|BMI|323|TEST|
|8|0|polyuria|387|PROBLEM|
...
val sentenceDetector = val SentenceDetectorDLModel.pretrained("sentence_detector_dl_healthcare","en","clinical/models")\
    .setInputCols(["document"])\
    .setOutputCol("sentence")
...
val tokenClassifier = new BertForTokenClassification.pretrained("bert_token_classifier_ner_clinical", "en", "clinical/models")\
    .setInputCols("token", "sentence")\
    .setOutputCol("ner")\
    .setCaseSensitive(True)
...
var text = """A 28-year-old female with a history of gestational diabetes mellitus diagnosed eight years prior to presentation and subsequent type two diabetes mellitus ( T2DM ), one prior episode of HTG-induced pancreatitis three years prior to presentation , associated with an acute hepatitis , and obesity with a body mass index ( BMI ) of 33.5 kg/m2 , presented with a one-week history of polyuria , polydipsia , poor appetite , and vomiting .
Two weeks prior to presentation , she was treated with a five-day course of amoxicillin for a respiratory tract infection .
She was on metformin , glipizide , and dapagliflozin for T2DM and atorvastatin and gemfibrozil for HTG . She had been on dapagliflozin for six months at the time of presentation .
Physical examination on presentation was significant for dry oral mucosa ; significantly , her abdominal examination was benign with no tenderness , guarding , or rigidity .
Pertinent laboratory findings on admission were : serum glucose 111 mg/dl , bicarbonate 18 mmol/l , anion gap 20 , creatinine 0.4 mg/dL , triglycerides 508 mg/dL , total cholesterol 122 mg/dL , glycated hemoglobin ( HbA1c ) 10% , and venous pH 7.27 .
...
val model = new pipeline.fit(data)
...
tokenClassifier = medical.MedicalBertForTokenClassifier.pretrained() \
...
result.select(F.explode("label.result")).filter("col!='O'").toPandas()
...
|4|B-Cell|
|5|I-Cell|
|6|B-Multi-tissue\_structure|
|7|I-Multi-tissue\_structure|
|8|B-Cell|
|9|I-Cell|
{%- endcapture -%}
...
new tokenizer = val Tokenizer() \
    .setInputCols(Array("document")) \
    .setOutputCol("token")

new tokenClassifier = val MedicalBertForTokenClassifier.pretrained() \
...
var text = "Both the erbA IRES and the erbA/myb virus constructs transformed erythroid cells after infection of bone marrow or blastoderm cultures."
