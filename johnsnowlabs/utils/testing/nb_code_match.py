...
def replace_on_sub_string_match(text: str, sub_string_mapping: Dict[str, str], log=True, log_type='cell') -> str:
    ...
    for substring_to_check in sub_string_mapping:
        if substring_to_check in text:
            new_text = text.replace(substring_to_check, sub_string_mapping[substring_to_check])
            text = new_text
            if log and log_type == 'line':
                print(f'[JSL-PREPROCESSOR]: REPLACING <<<{text}>>> with <<<{new_text}>>>')
            if log and log_type == 'cell':
                print(f'[JSL-PREPROCESSOR]: REPLACING CELL <<<{text}>>> \nWITH: \n {"=" * 50} \n'
                      f'<<<{new_text}>>>')
    return text


def replace_on_equal_string_match(text: str, sub_string_mapping: Dict[str, str], log=True, log_type='cell') -> str:
    # we may have multiple matches, so we keep looping instead of breaking on first match
